...
void example08_setup(void) {
  example08_class = class_new(gensym("example08"),
    ...
    sizeof (t_example08), CLASS_DEFAULT, 0);
  // leftmost inlet will trigger methods upon
  // these messages.
  class_addmethod(example08_class, (t_method) example08_start, gensym("start"), 0);
  class_addmethod(example08_class, (t_method) example08_start, gensym("bang"),  0);
